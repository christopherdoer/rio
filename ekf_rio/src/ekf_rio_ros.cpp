...
bool EkfRioRos::initImu(const ImuDataStamped& imu_data)
{
  ...
  {
    initialized_            = ekf_rio_filter_.init(imu_init_);
...
bool EkfRioRos::initBaro(const sensor_msgs::FluidPressure& baro)
{
  if (initialized_)
  {
  ...
  {
    baro_init_vec_.emplace_back(baro_altimeter_.calculate_rel_neg_height(baro, baro_h_0_));
  }
}
...
void EkfRioRos::iterate()
{
  ...
  if (!queue_baro_.empty())
  {
    ...
    {
      initBaro(baro_temp_msg);
      queue_baro_.pop();
    }
    else if (ekf_rio_filter_.getTimestamp() >= baro_temp_msg.header.stamp)
    {
      queue_baro_.pop();
